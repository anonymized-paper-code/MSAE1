...
class SimpleHGNLayer(nn.Module):
    ...
    def __init__(
        ...
        self.edge_dict = list(edge_dict.keys())
    ...
    def forward(self, all_type_edge_src_node_feature_adj_dict, res_attn, srctype=None, level_num=None, recorders=None):
...
class SimpleHGN(nn.Module):
    def __init__(self, Model_Config_dict):
        ...
        edge_feats_len = Model_Config_dict['edge_feats_len']
        self.Target_Node_Type = Model_Config_dict['Target_Node_Type']
        self.task_to_node = Model_Config_dict['task_to_node']
...
#         self.all_relation_list.append('self_loop')
        ...
        self.task_name = Model_Config_dict['task_name']
        self.multi_tower = Model_Config_dict['multi_tower']
        self.dif_e_imp = Model_Config_dict['dif_e_imp']
        self.node_list = list(node_dict.keys())
        self.edge_list = list(edge_dict.keys())
...
#         # 不同task meta path重要性不同
#         self.e_imp = nn.Pramnn.Parameter(torch.zeros(len(self.edge_list))
        
        # 有多少种类型的元路径，每种元路径有多少条，就生成多少个GAT
        self.edge_GAT =  nn.ModuleList()
        # input projection
        ## TODO: 第一层加node_redidual
        self.edge_GAT.append(SimpleHGNLayer(edge_feats_len = edge_feats_len, 
        ...
        # middle projection
        self.edge_GAT.extend([SimpleHGNLayer(edge_feats_len = edge_feats_len, 
        ...
        # final projection
        self.final_edge_GAT = nn.ModuleList()
        if self.multi_tower:
            for ind, _ in enumerate(self.task_name):
                self.final_edge_GAT.append(SimpleHGNLayer(edge_feats_len = Model_Config_dict['edge_feats_len'], 
                                               in_features_len = GAT_hid_len * self.head_num,
                                               out_features_len = self.class_num[ind],
                                               nhead = 1,
                                               edge_dict = edge_dict,
                                               edge_type_each_node = Model_Config_dict['edge_type_each_node'],                
                                               node_residual=True,
            ...
            self.final_edge_GAT.append(SimpleHGNLayer(edge_feats_len = Model_Config_dict['edge_feats_len'], 
                                               in_features_len = GAT_hid_len * self.head_num,
                                               out_features_len = self.class_num[0],
                                               nhead = 1,
                                               edge_dict =edge_dict,
                                               edge_type_each_node = Model_Config_dict['edge_type_each_node'],            
                                               node_residual=True,
    ...
    def reset_parameters(self):
        ...
        for final_edge_GAT_i in self.final_edge_GAT:
            final_edge_GAT_i.reset_parameters()
...
#                 adj_begin_index[srctype] = adj.shape[1]
            ...
            all_type_edge_src_node_feature_adj_dict[srctype]['Adj'].append(adj)
            # 生成edge_type的对应index
            tmp_edge_index = self.edge_list.index(etype)
            tmp_edge_index = torch.tensor([tmp_edge_index]*adj.shape[1])
            if self.args_cuda:
                tmp_edge_index = tmp_edge_index.cuda()
            all_type_edge_src_node_feature_adj_dict[srctype]['tmp_edge_index'].append(tmp_edge_index)
        ...
        for i in range(self.layer_num):
            ...
            for srctype in all_type_edge_src_node_feature_adj_dict:
                # 导入GAT获取结果
                inter_srctype_feaure[srctype] = self.edge_GAT[i](all_type_edge_src_node_feature_adj_dict[srctype], all_type_edge_src_node_feature_adj_dict[srctype]['res_attn'], srctype, i, recorders)
        ...
        # final layer for different task
        final_srctype_feaure = []
        for ind, edge_GAT in enumerate(self.final_edge_GAT):
            inter_srctype_feaure = {}
            for srctype in all_type_edge_src_node_feature_adj_dict:
                # 导入GAT获取结果
                inter_srctype_feaure[srctype] = edge_GAT(all_type_edge_src_node_feature_adj_dict[srctype], all_type_edge_src_node_feature_adj_dict[srctype]['res_attn'])
            final_srctype_feaure.append(inter_srctype_feaure)

        ## (不一样) TODO: check final_srctype_feaure append的结果是否都一样 
        if not self.multi_tower:
            final_srctype_feaure = final_srctype_feaure*len(self.task_name)
        ...
        for ind, task in enumerate(self.task_name):
            ...
            h_prime = final_srctype_feaure[ind][self.task_to_node[task]][0]
